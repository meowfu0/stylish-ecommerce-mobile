...
export default function ProductDetailsScreen() {
  ...
  const galleryWidth = desktopWeb
    ? Math.min(620, contentWidth * 0.54)
    : contentWidth;
  const detailsWidth = desktopWeb
    ? contentWidth - galleryWidth - spacing.xl
    : contentWidth;
  const similarCardWidth = desktopWeb
  ...
  return (
    ...
    >
      ...
      <Animated.View style={[{ flex: 1 }, entranceStyle]}>
        ...
        >
          <ScrollReveal
            className={desktopWeb ? "flex-row items-start gap-xl pt-xl" : ""}
            style={{ width: contentWidth }}
          >
            <ProductGallery images={product.gallery} width={galleryWidth} />

            <View style={{ width: detailsWidth }}>
              <View className={desktopWeb ? "" : "mt-md"}>
              ...
              <View
                className="mt-[12px] h-[72px] justify-center rounded-md bg-brand-delivery px-[24px]"
          ...
          >
            ...
            >
              {SIMILAR_PRODUCTS.map((similarProduct, index) => (
                <ScrollReveal
